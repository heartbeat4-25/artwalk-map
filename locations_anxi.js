const districtData = [
  {
    name: "五峰山茶馆",
    lng: 118.183973,
    lat: 25.027294,
    desc: "位于安溪凤山绿道附近，环境幽静，是当地颇受欢迎的休闲茶馆。",
    images: [
        "images/cards/anxi/wufeng1.png",
        "images/cards/anxi/wufeng2.png",
        "images/cards/anxi/wufeng3.png"
    ]
  },
  {
    name: "清隐茶馆",
    lng: 118.189509,
    lat: 25.065298,
    desc: "隐于凤山绿道，主打围炉煮茶、罐罐烤奶等特色茶饮，假期人气爆棚，是安溪夜间经济代表之一。",
    images: [
        "images/cards/anxi/qingyin1.png",
        "images/cards/anxi/qingyin2.png",
        "images/cards/anxi/qingyin3.png"
    ]
  },
  {
    name: "将相和山庄",
    lng: 118.210572,
    lat: 25.066016,
    desc: "",
    images: [
        "images/cards/anxi/jiangxianghe1.png",
        "images/cards/anxi/jiangxianghe2.png"
    ]
  },
  {
    name: "芳松里小院",
    lng: 118.20679199999995,
    lat: 25.0252,
    desc: "",
    images: [
        "images/cards/anxi/fangsongli1.png",
        "images/cards/anxi/fangsongli2.png",
        "images/cards/anxi/fangsongli3.png"
    ]
  },
  {
    name: "静心茶馆(安溪凤山风景旅游区店)",
    lng: 118.185227,
    lat: 25.070434,
    desc: "",
    images: [
        "images/cards/anxi/jingxin1.png",
        "images/cards/anxi/jingxin2.png"
    ]
  },
  {
    name: "冠和茶馆",
    lng: 118.188442,
    lat: 25.04341,
    desc: "",
    images: [
        "images/cards/anxi/guanhe1.png"
    ]
  },
  {
    name: "茶香小院",
    lng: 118.206529,
    lat: 25.022184,
    desc: "",
    images: [
        "images/cards/anxi/chaxiang1.png",
        "images/cards/anxi/chaxiang2.png",
        "images/cards/anxi/chaxiang3.png",
        "images/cards/anxi/chaxiang4.png"
    ]
  },
  {
    name: "阅谈共享茶室",
    lng: 118.175107,
    lat: 25.059893,
    desc: "",
    images: [
        "images/cards/anxi/yuedu1.png",
        "images/cards/anxi/yuedu2.png"
    ]
  },
  {
    name: "七号茶楼",
    lng: 118.159291,
    lat: 25.062268,
    desc: "",
    images: [
        "images/cards/anxi/qihao1.png"
    ]
  },
  {
    name: "清溪茶驿·龙湖",
    lng: 118.189462,
    lat: 25.049323,
    desc: "",
    images: [
        "images/cards/anxi/qingxi1.png",
        "images/cards/anxi/qingxi2.png",
        "images/cards/anxi/qingxi3.png"
    ]
  },
  {
    name: "孔雀山庄",
    lng: 118.168691,
    lat: 25.041245,
    desc: "",
    images: [
        "images/cards/anxi/kongque1.png",
        "images/cards/anxi/kongque2.png"
    ]
  },
  {
    name: "渝风堂",
    lng: 118.186414,
    lat: 25.027414,
    desc: "",
    images: [
        "images/cards/anxi/yufeng1.png",
        "images/cards/anxi/yufeng2.png",
        "images/cards/anxi/yufeng3.png"
    ]
  },
  {
    name: "八马茶业",
    lng: 118.093979,
    lat: 24.986503,
    desc: "",
    images: [
        "images/cards/anxi/bama1.png"
    ]
  },
  {
    name: "溪禾老茶馆",
    lng: 118.210296,
    lat: 25.085068,
    desc: "",
    images: [
        "images/cards/anxi/xihe1.png",
        "images/cards/anxi/xihe2.png",
        "images/cards/anxi/xihe3.png"
    ]
  },
  {
    name: "约茶吧",
    lng: 118.155542,
    lat: 25.067027,
    desc: "",
    images: [
        "images/cards/anxi/yuecha1.png"
    ]
  },
  {
    name: "九玖茶舍",
    lng: 118.103472,
    lat: 25.024707,
    desc: "",
    images: [
        "images/cards/anxi/jiujiu1.png"
    ]
  },
  {
    name: "阆苑岩茶馆",
    lng: 118.155683,
    lat: 25.032282,
    desc: "",
    images: [
        "images/cards/anxi/langyuan1.png",
        "images/cards/anxi/langyuan2.png"
    ]
  },
  {
    name: "素音茶舍",
    lng: 118.18815199999995,
    lat: 25.06227,
    desc: "安溪素音茶舍，坐落于泉州市安溪县凤城镇凤仪园二巷的静谧小巷中。这里远离喧嚣闹市，环境清雅幽静，茶香氤氲，伴随素音袅袅，带来一份独属于茶舍的安宁与惬意。\n" +
        "\n" +
        "茶舍以弘扬安溪茶文化为宗旨，精选本地铁观音等优质名茶，结合传统茶艺表演与现代雅致空间，为来客提供一场关于味觉与心灵的双重体验。无论是与三五知己品茗谈心，还是独自静坐、品味茶香，素音茶舍都能带来一种“茶味即人生”的悠然意境。\n" +
        "\n" +
        "在这里，茶不仅是一种饮品，更是一种生活方式，一种心灵修养。素音茶舍愿以一盏好茶，邀您慢下来，感受安溪的茶香与文化底蕴。",
    images: [
        "images/cards/anxi/suyin1.png",
        "images/cards/anxi/suyin2.png",
        "images/cards/anxi/suyin3.png"
    ]
  },
  {
    name: "国心绿谷",
    lng: 117.97486800000001,
    lat: 25.183596000000005,
    desc: "国心绿谷茶庄园，位于安溪县尚卿乡、五阆山主峰，占地面积近万亩，海拔在750-1250米之间。这里四季有花，三季有果，鸟语花香，冬夏成荫，土壤肥沃，云雾缭绕。\n" +
        "2012年以来，茶庄园以打造以“国家AAA级生态茶庄园旅游景区”为目标，建成现代化加工厂，特色度假酒店、农家乐、陆羽观景台、海拔1180米高的一号峰、二号峰景观台，以及低碳型停车场。游客可步行、踩单车或坐景区观光车，一路欣赏美景、畅谈人生。\n" +
        "地址：安溪县尚卿乡黄岭村",
    images: [
        "images/cards/anxi/guoxin1.png",
    ]
  },
  {
    name: "溪禾山铁观音文化园",
    lng: 118.20924500000001,
    lat: 25.083272000000004,
    desc: "",
    images: [

    ]
  },
  {
    name: "高雄建发茶庄园",
    lng: 117.92513000000008,
    lat: 24.94309800000001,
    desc: "",
    images: [

    ]
  },
  {
    name: "华祥苑茶庄园",
    lng: 117.68023199999993,
    lat: 24.964947999999996,
    desc: "华祥苑茶庄园拥有从种植、生产、加工完全按生态标准进行生产的庄园茶基地，6000多平米铁观音初制加工厂，种茶采茶体验区，古法制茶体验区、品茗室，以及茶文化馆、茶艺表演区、农产品交易区、观光木屋、特色农家菜餐饮区、停车场等，成为集茶叶种植、加工、观光、休闲于一体的综合性茶文化基地。\n" +
        "地址：安溪县龙涓乡珠塔村",
    images: [
        "images/cards/anxi/huaxiangyuan1.png",
    ]
  },
  {
    name: "陈素全茶馆",
    lng: 117.75023199999993,
    lat: 25.244947999999996,
    desc: "",
    images: [

    ]
  },
  {
    name: "龙通茶庄园",
    lng: 117.86023199999993,
    lat: 25.364947999999996,
    desc: "",
    images: [

    ]
  },
  {
    name: "云岭茶庄园",
    lng: 117.84023199999993,
    lat: 25.064947999999996,
    desc: "",
    images: [

    ]
  },
];
